...
# Remove old gazebo
sudo apt remove gz-harmonic -y && sudo apt autoremove -y || { echo "Error: Failed to remove old gazebo harmonic";exit 1; }
...
sudo apt-get install gz-harmonic -y || { echo "Error: Failed to install Gazebo."; exit 1; }
